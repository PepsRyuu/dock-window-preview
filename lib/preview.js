...
module.exports = {
    init () {
        ...
        // TODO: Potentially swap out for https://developer.apple.com/documentation/appkit/nsevent/1535472-addglobalmonitorforevents
        // Need to consider how to reduce overall CPU usage by taking advantage of OS level events.
        // Is there any way to listen to mouse hovers over the dock?
        setInterval(() => {
            ...
            if (el && el.type === 'AXApplicationDockItem') {
                ...
                if (el.running) {
                    ...
                    let found = windows.filter(w => w.owner === name).filter(w => {
                        // This checks to see if the window is a real window
                        return axlib.AXCheckIfStandardWindow(w.pid, w.window);
                    });
